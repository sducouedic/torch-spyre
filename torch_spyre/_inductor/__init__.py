        ...
        def _wrapper(gm, example_inputs, *args, **kwargs):
            ...
            try:
                if uses_spyre:
                    ...
                    # references into FxGraphCache.  Spyre's compiled wrappers are
                    # compatible with FxGraphCache (SpyreSDSCKernelRunner is
                    # picklable), but AOTAutogradCache itself is not exercised for
                    # Spyre graphs because the Spyre compile path goes through the
                    # compile_fx wrapper rather than AOTAutograd.  If a user has
                    # enabled it explicitly, warn rather than silently producing a
                    # broken or stale cache entry.
                    ...
                    if getattr(_ftn_config, "enable_autograd_cache", False):
                        ...
                        warnings.warn(
                            "torch._functorch.config.enable_autograd_cache is True, "
                            "but AOTAutogradCache is not supported for Spyre graphs. "
                            "The AOTAutogradCache layer will be bypassed for this graph. "
