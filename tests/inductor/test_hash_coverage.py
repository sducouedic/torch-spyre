...
Each test class covers one category from the Hash Coverage Table:
  ...
  TestUseSymbolsFlagInExtra    — bundle_symbolic_args is baked into compile config
"""
...
def _make_op_spec(
    ...
    """Return a minimal OpSpec-like MagicMock for a single-input elementwise op."""
    ...
    spec = OpSpec(
        ...
        # iteration_space: {sym: (range, work_division_split)}
...
def _hash(specs, use_symbols: bool = False):
    ...
    _get_compile_config is also patched — it is lru_cache'd, so patching
    bundle_symbolic_args alone would not vary the hash across calls.
    """
    import json
    from torch_spyre.execution.kernel_cache import compute_specs_hash

    compile_config = json.dumps(
        {"bundle_symbolic_args": use_symbols, "test": True}, sort_keys=True
    )
    with (
        patch(
        ...
        patch(
            "torch_spyre.execution.kernel_cache._get_compile_config",
            return_value=compile_config,
        ),
        patch(
...
# bundle_symbolic_args flag affects hash
class TestUseSymbolsFlagInExtra(unittest.TestCase):
    """bundle_symbolic_args is baked into the compile config that feeds
    code_hash so that changing it invalidates the cache."""

    def test_use_symbols_true_vs_false_produce_different_hashes(self):
        op = _make_op_spec()

        h_false = _hash([op], use_symbols=False)
        h_true = _hash([op], use_symbols=True)

        self.assertNotEqual(
            h_false,
            h_true,
            "bundle_symbolic_args=True vs False must produce different hashes.",
        )
