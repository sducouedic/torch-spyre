...
def _get_dxp_version() -> str:
    ...
    missing, or the ``ibm-deeptools`` entry cannot be found — callers must
    disable caching rather than risk stale cache hits with an unknown
    compiler version.
    """
    ...
    if "ibm-deeptools" not in versions:
        raise RuntimeError(
            f"'ibm-deeptools' entry not found in {components_file!r}; "
            "cannot determine compiler version for cache key. "
            "Set SPYRE_KERNEL_CACHE=0 to run without caching."
        )
    # Return a deterministic combined string; flex may legitimately be absent
    # on some dev builds — mark it explicitly rather than silently omitting.
    return (
        f"deeptools={versions['ibm-deeptools']};flex={versions.get('flex', 'unknown')}"
    )
...
def _get_compile_config() -> str:
    ...
    relevant = {
        # Core architecture / device knobs
        "sencores": c.sencores,
        ...
        "lx_planner_relayout": c.lx_planner_relayout,
        # Tiling / emission
        "enable_reduction_tiling": c.enable_reduction_tiling,
        "core_id_k_fast_emission": c.core_id_k_fast_emission,
        "ignore_span_overflow_hints": c.ignore_span_overflow_hints,
        # Environment variables read directly by dxp_standalone / the runtime
