...
class TestHbmPoolPlanningE2E(InductorTestCase):
    ...
    def test_pool_alloc_scoped_per_bundle_across_fallback_boundary(self):
        ...
        # cache path it is never called at all -- the compile dir is
        # allocated under the cache root and then renamed to its final
        # <cache_root>/<key>/ location by commit_compile_dir(). Record the
        # directory sdsc() ultimately settled on by wrapping the runner it
        # returns: code_dir is the post-commit path on every path.
